...
function generatePO() {
  ...
  (function(){
    ...
    if (qty > 0) _poRows.push({
      product_id:{{ r.id }}, name:'{{ r.name|e }}',
      flavor:'{{ r.flavor|e }}', type:'{{ r.type|e }}',
      stock:{{ r.qty }}, qty
    });
  })();
  {% endfor %}
...
.ro-back{text-decoration:none;color:var(--brand);font-size:.82rem;font-weight:700;display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border:1.5px solid var(--brand);border-radius:99px;transition:.15s;}
...
.po-card{background:#fff;border:1.5px solid var(--border);border-radius:var(--radius);margin-bottom:20px;box-shadow:var(--shadow);overflow:hidden;}
.po-card-head{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;padding:14px 18px;background:#f8f7ff;border-bottom:1.5px solid var(--border);cursor:pointer;user-select:none;}
...
.recv-input{width:70px;text-align:center;border:1.5px solid var(--brand);border-radius:8px;padding:5px 4px;font-size:.85rem;font-weight:700;outline:none;}
...
.btn-receive{background:var(--green);color:#fff;border:none;padding:10px 22px;border-radius:var(--radius);font-size:.85rem;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:7px;transition:.2s;}
...
.btn-delete{background:#fff;color:var(--red);border:1.5px solid var(--red);padding:10px 18px;border-radius:var(--radius);font-size:.85rem;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:7px;transition:.2s;}
    ...
    <div class="po-card-body" id="body-{{ po.id }}" style="display:none;">
      <div>
        <table class="verify-table">
          <thead>
            <tr>
              <th>#</th><th>Product</th><th>Flavor / Type</th>
      ...
      <div class="po-actions">
        ...
        {% else %}
        <button class="btn-receive" disabled style="background:#94a3b8;">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (first) { const id = first.id.replace('po-card-',''); toggleCard(id); }
});
</script>
{% endblock %}
"""
